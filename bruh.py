...
class BuiltInFunction(BaseFunction):
    ...
    def execute_length(self, exec_ctx):
        ...
        if isinstance(value, String):
            to_return = str(exec_ctx.symbol_table.get("value")).strip('"').replace(" ","")
            return RTResult().success(Number(len(to_return)))
    ...
    def execute_sqrt(self, exec_ctx):
        ...
        if isinstance(value, Number):
            to_return = float(str(value).strip('"'))
            return RTResult().success(Number(to_return ** 0.5))
        return RTResult().failure(RTError(
            self.pos_start, self.pos_end,
    ...
    def execute_reverse(self, exec_ctx):
        ...
        if isinstance(value, List):
            ...
            var = value.value[::-1]
            return RTResult().success(String(var))
